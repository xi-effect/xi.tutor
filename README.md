...
2. Экспортировать их в файле `src/locales/index.ts`
3. Добавить в `package.json` пакета export `"./locales": "./src/locales/index.ts"` (лёгкий entry только с JSON — без UI)
4. Опционально реэкспортировать из корня пакета для удобства
5. Добавить `import('my.package/locales')` и регистрацию namespace в `apps/xi.web/src/config/i18n.ts`

> Важно: в `i18n.ts` импортируйте именно `package/locales`, а не корень пакета — иначе в критический путь старта может попасть UI-граф модуля.
...
Переключение языка доступно в настройках профиля → **Персонализация**. Выбор сохраняется в `localStorage` (`xi_language`) и применяется сразу (включая `html[lang]` и локаль dayjs).
