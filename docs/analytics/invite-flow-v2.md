...
«Обновить ссылку» (иконка рядом с основной кнопкой) — не аддитивное создание,
а замена: текущее приглашение удаляется (`DELETE`, существующий метод API) и
сразу создаётся новое, поэтому у репетитора всегда ровно одно активное
неиспользованное приглашение из этого сценария и общее число приглашений не
растёт при повторных обновлениях. `student_invite_new_link_created` содержит
`previous_invite_id` — id удалённого приглашения.
