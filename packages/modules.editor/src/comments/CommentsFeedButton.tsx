...
import { getAllCommentThreads, getThreadMarkRange, getThreadMessages } from './commentQueries';
...
/** Кнопка в шапке редактора + выпадающая лента всех комментариев (прочитанных и непрочитанных). */
export const CommentsFeedButton = () => {
  ...
  // Реагируем на новые/удалённые треды и сообщения, в том числе пришедшие от других участников.
  ...
  const items = getAllCommentThreads(commentThreadsMap).map((thread) => ({
    thread,
    messages: getThreadMessages(commentMessagesMap, thread.id),
  }));
  ...
  const handleSelect = (thread: EditorCommentThread) => {
    setOpen(false);
  ...
  const feedTrigger = (
    <Button
      ...
      data-umami-event="editor-comments-feed"
      onClick={() => setOpen(true)}
    >
      <Inbox size="s" className={cn('h-4 w-4 lg:h-6 lg:w-6')} />
      {unreadCount > 0 && (
  ...
  const feedList = (
    <>
      <p className="text-text-secondary px-2 py-1 text-xs">{t('comments.inDocument')}</p>
      {sorted.length === 0 ? (
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>{feedTrigger}</PopoverTrigger>
      <PopoverContent
        align="end"
        side="bottom"
        sideOffset={12}
        data-comment-ui
        className="w-80 rounded-xl border-2 p-2"
      >
        {feedList}
      </PopoverContent>
    </Popover>
