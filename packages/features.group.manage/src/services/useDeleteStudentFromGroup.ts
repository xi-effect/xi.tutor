...
export const useDeleteStudentFromGroup = ({ classroom_id }: { classroom_id: string }) => {
    ...
    mutationFn: async (student_id: number) => {
      try {
        ...
        const response = await axiosInst({
          method: enrollmentsApiConfig[EnrollmentsQueryKey.DeleteStudentFromClassroom].method,
          url: enrollmentsApiConfig[EnrollmentsQueryKey.DeleteStudentFromClassroom].getUrl(
            classroom_id,
            student_id.toString(),
          ),
          headers: {
  ...
  return { ...deleteStudentFromGroupMutation };
};
