...
export const DrawCanvas = ({
  ...
  useEffect(() => {
    ...
    if (isReadonly) {
      ...
      const handlePointerDown = (e: PointerEvent) => {
        ...
        if (
          target.closest('.dr-shape') ||
          target.closest('.dr-selected') ||
          target.closest('[data-shape-id]') ||
          target.closest('.dr-svg-container')
        ) {
          e.preventDefault();
          e.stopPropagation();
  ...
  useEffect(() => {
    ...
    const sync = () => {
      const presence = store.get(id as DrInstancePresence['id']) as DrInstancePresence | undefined;
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => {
      if (useDrawStore.getState().inputMode !== 'auto') return;
          ...
          <Draw
            ...
            onMount={(editor) => {
              ...
              editor.sideEffects.registerBeforeChangeHandler('instance', (_, next) => {
                const mode = useDrawStore.getState().inputMode;
