    ...
    <>
      ...
      >
        <DrawerPrimitive.Portal>
          <DrawerPrimitive.Overlay
            className={cn('bg-background-overlay fixed inset-0', boardDropdownZClass)}
          />
          <DrawerPrimitive.Content
            data-board-cloud-files-drawer
            className={cn(
              'bg-background-surface border-border-default fixed inset-y-0 right-0 flex h-full w-full max-w-lg flex-col rounded-tl-2xl rounded-bl-2xl border-l outline-none',
              boardDropdownZClass,
            )}
          >
            <div className="border-border-default flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3">
              <DrawerPrimitive.Title className="font-playfair text-text-primary m-0 text-xl font-medium">
                {t('navbar.cloudFilesTitle')}
              </DrawerPrimitive.Title>
              <DrawerPrimitive.Description className="sr-only">
                {t('navbar.cloudFilesDescription')}
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2 px-4 py-2.5">
              ...
              {filtersActive ? (
                <Button
                  ...
                  data-umami-event="board-cloud-files-reset-all"
                >
            ...
            <div
              ...
              aria-label={t('navbar.cloudFilesTitle')}
              onScroll={handleListScroll}
            >
              {isLoading ? (
                <p className="text-s-base text-text-secondary py-10 text-center">
                  {t('navbar.cloudFilesLoading')}
                </p>
              ) : isError ? (
                <p className="text-s-base text-text-secondary py-10 text-center">
                  {t('navbar.cloudFilesError')}
                </p>
              ) : !files.length ? (
                <p className="text-s-base text-text-secondary py-10 text-center">
                  {t('navbar.cloudFilesEmpty')}
                </p>
              ) : !filteredFiles.length ? (
      ...
      <FilePreviewModal
        ...
        primaryAction={
          currentPreviewFile
            ? {
                label: t('navbar.cloudAddToBoard'),
...
export const CloudFilesDrawer = (props: CloudFilesDrawerProps) => (
  <LibraryTagsUiProvider>
    <div className={boardChromeZClass}>
      <CloudFilesDrawerContent {...props} />
    </div>
  </LibraryTagsUiProvider>
);
