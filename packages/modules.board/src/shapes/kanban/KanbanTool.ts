import { createShapeId, StateNode, DrShapeId, VecLike } from '@ibodr/draw';
import { createDefaultColumns, KANBAN_DEFAULT_HEIGHT, KANBAN_DEFAULT_WIDTH } from './KanbanShape';

const CLICK_DRAG_THRESHOLD = 4;
const MIN_DRAG_SIZE = 480;

export class KanbanTool extends StateNode {
  static override id = 'kanban';
  ...
  override onPointerUp() {
    ...
    if (!shape || !this.editor.isShapeOfType(shape, 'kanban')) {
    ...
    const w = isClick ? KANBAN_DEFAULT_WIDTH : Math.max(MIN_DRAG_SIZE, dragW);
    const h = isClick ? KANBAN_DEFAULT_HEIGHT : Math.max(MIN_DRAG_SIZE, dragH);
