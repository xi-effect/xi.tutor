import { ClassroomsQueryKey, ClassroomTutorResponseSchema } from 'common.api';
...
export const useGetClassroom = (classroomId: number, disabled?: boolean) => {
  const { data, isError, isLoading, ...rest } = useQuery({
    ...
    queryFn: async () => {
      ...
      const url = `${env.VITE_SERVER_URL_BACKEND}/api/protected/classroom-service/roles/tutor/classrooms/${classroomId}/`;
