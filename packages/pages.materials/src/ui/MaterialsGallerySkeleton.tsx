export const MaterialsGallerySkeleton = ({ count = 8 }: { count?: number }) => (
  <div className="grid grid-cols-1 gap-5 pb-4 sm:grid-cols-[repeat(auto-fill,minmax(300px,1fr))]">
    {Array.from({ length: count }).map((_, i) => (
      <div
        key={i}
        className="bg-background-surface flex h-44 w-full flex-col rounded-2xl p-5 shadow-[0px_2px_8px_0px_rgba(0,0,0,0.08)]"
      >
        ...
        <div className="mt-auto flex w-full flex-col gap-1 pt-3">
          ...
          <div className="bg-background-subtle h-4 w-36 animate-pulse rounded" />
        </div>
      </div>
    ))}
  </div>
