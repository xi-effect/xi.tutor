...
function nums(text: string) {
  ...
  const source =
    listed?.[1] ?? text.match(/(-?\d+(?:[.,]\d+)?(?:\s*[,;]\s*-?\d+(?:[.,]\d+)?){3,})/)?.[1];
  if (!source) return [];
  return [...source.matchAll(/-?\d+(?:[.,]\d+)?/g)].map((match) =>
...
export const sequenceInterpreter: MathInterpreterModule = {
  ...
  interpret(input) {
    ...
    const arithmetic = values
      .slice(1)
      .every((value, index) => Math.abs(value - values[index] - d) < 1e-9);
    const q = values[0] !== 0 ? values[1] / values[0] : NaN;
    const geometric =
      Number.isFinite(q) &&
      values
        .slice(1)
        .every((value, index) => values[index] !== 0 && Math.abs(value / values[index] - q) < 1e-9);
    return [
      {
        id: 'sequence',
