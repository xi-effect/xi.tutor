...
- `src/ui/CompactView/CompactCall.tsx` - `handleMaximize`
...
2. **Сохранение состояния доски:** При переключении на `full` mode информация о доске (`activeBoardId`, `activeClassroom`) не очищается, чтобы пользователь мог вернуться на доску.
