...
export const ScheduleKanban: FC<ScheduleKanbanProps> = ({
  ...
  return (
    <div className="flex h-[calc(100vh-72px)] flex-1 flex-col overflow-hidden">
      ...
      {/* Один общий скролл: контент не короче области — пустые колонки на всю высоту экрана */}
      <div
        ref={scrollAreaRef}
        className="flex min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-auto [scrollbar-gutter:stable]"
      >
        {/* pr на внутренней обёртке: у overflow-y-auto padding на самом скролле часто не отодвигает контент от overlay-скроллбара */}
        <div
          ...
          <div className="grid flex-1 items-stretch gap-x-7 pb-4" style={{ gridTemplateColumns }}>
            {segments.map((seg) => {
              ...
              return (
                ...
                >
                  {/* relative h-full — чтобы min(100%, …) в высоте sticky считалась от ячейки строки */}
